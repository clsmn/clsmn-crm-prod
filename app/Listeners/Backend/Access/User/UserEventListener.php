...
class UserEventListener
{
     ...
     * @param $event
     */
    public function onCheckIn($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->user->id)
            ->withText('<strong>{user}</strong> trans("history.backend.users.check_in")')
            ->withIcon('sign-in')
            ->withClass('bg-green')
            ->withAssets([
                'user_link' => ['admin.access.user.show', $event->user->name, $event->user->id],
    ...
    public function onCheckOut($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->user->id)
            ->withText('<strong>{user}</strong> trans("history.backend.users.check_out")')
            ->withIcon('sign-out')
            ->withClass('bg-yellow')
            ->withAssets([
                'user_link' => ['admin.access.user.show', $event->user->name, $event->user->id],
     ...
     * @param $event
     */
    public function onFetchLeads($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->user->id)
            ->withText('<strong>{user}</strong> trans("history.backend.users.fetch_leads")')
            ->withIcon('download')
            ->withClass('bg-blue')
            ->withAssets([
                'user_link' => ['admin.access.user.show', $event->user->name, $event->user->id],
    ...
    public function subscribe($events)
    {
        ...
        $events->listen(
            \App\Events\Backend\Access\User\UserCheckIn::class,
            'App\Listeners\Backend\Access\User\UserEventListener@onCheckIn'
        );

        $events->listen(
            \App\Events\Backend\Access\User\UserCheckOut::class,
            'App\Listeners\Backend\Access\User\UserEventListener@onCheckOut'
        );

        $events->listen(
            \App\Events\Backend\Access\User\UserFetchLeads::class,
            'App\Listeners\Backend\Access\User\UserEventListener@onFetchLeads'
        );
    }
}
